...
def set_laplace_update(state: np.ndarray, learning_rate: float = 1.0) -> None:
    """
    Apply the Laplace operator to update the state in place.

    This function implements a finite difference approximation of the Laplace
    operator using a 5-point stencil. It computes the average of the four
    neighboring values (left, right, bottom, top) for each interior point.

    Args:
        state (np.ndarray): Current state array of shape (n, m)
        learning_rate (float): Relaxation factor for the update.
